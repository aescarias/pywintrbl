...
def get_path_from_hkey(handle: int) -> str:
    ...
    # ULONG   Length,
    # PULONG  ResultLength
    ntdll.NtQueryKey.argtypes = (ctypes.c_void_p, ctypes.c_byte, ctypes.c_void_p, ctypes.c_ulong, ctypes.c_void_p)
    ntdll.NtQueryKey.restype = ctypes.c_ulong
    
    # First request should fail with a BUFFER_TOO_SMALL status.
    # This allows to get the length of the key
    result_length = ctypes.c_void_p()
    status = ntdll.NtQueryKey(handle, 0x3, 0, 0, ctypes.byref(result_length))
    assert status == 0xC0000023

    # The buffer size is tne result length + 2 bytes
    buffer_size = (result_length.value + 2) // ctypes.sizeof(ctypes.c_wchar)
    buffer = ctypes.create_unicode_buffer(buffer_size)

    # 0x3 is KEY_NAME_INFORMATION
    status = ntdll.NtQueryKey(handle, 0x3, buffer, result_length.value + 2, ctypes.byref(result_length))
    assert status == 0x0 # this should not fail

    # first two chars are for length, the next chars are the actual key
    # (the string is null-terminated hence the -1) 
    return buffer[2:-1]
...
def get_friendly_hkey_path(path: str) -> str:
    ...
    raise ValueError(f"Cannot parse object name: {path}")
